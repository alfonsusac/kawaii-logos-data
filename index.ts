...
export const getImages = async (): Promise<DataImage[]> => [
  ...
  ...(await (async () => {
    const i = await Promise.all(
      scrapedImages.map(async scrapedImage => {
        const cwd = `${import.meta.dir}/cloned/${scrapedImage.ownerRepoPath}`;
        if (!existsSync(cwd)) {
          await Bun.$`git clone https://github.com/${
            scrapedImage.ownerRepoPath
          }.git ${import.meta.dir}/cloned/${scrapedImage.ownerRepoPath}`.cwd(
            "./scrape"
          );
        } else {
